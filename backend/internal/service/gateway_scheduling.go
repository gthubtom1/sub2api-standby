		...
		if len(routingCandidates) > 0 {
			...
			if len(routingAvailable) > 0 {
				...
				sort.SliceStable(routingAvailable, func(i, j int) bool {
					...
					if cmp := compareStandbyProbeFreshness(a.account, b.account, time.Now()); cmp != 0 {
						return cmp < 0
					}
					switch {
...
func sortAccountsByPriorityAndLastUsed(accounts []*Account, preferOAuth bool) {
	...
	sort.SliceStable(accounts, func(i, j int) bool {
		...
		// Prefer standby-probed accounts so cold bad accounts are not chosen first.
		if cmp := compareStandbyProbeFreshness(a, b, now); cmp != 0 {
			return cmp < 0
		}
		switch {
...
func sameAccountWithLoadGroup(a, b accountWithLoad) bool {
	...
	if IsStandbyProbeFresh(a.account, time.Now()) != IsStandbyProbeFresh(b.account, time.Now()) {
		return false
	}
...
func sameAccountGroup(a, b *Account) bool {
	...
	// Keep verified and unverified accounts in separate shuffle buckets.
	if IsStandbyProbeFresh(a, time.Now()) != IsStandbyProbeFresh(b, time.Now()) {
		return false
	}
