...
func (s *AccountTestService) testClaudeAccountConnection(c *gin.Context, account *Account, modelID string) error {
	...
	if resp.StatusCode != http.StatusOK {
		...
		s.applyTestUpstreamFailure(ctx, account, resp.StatusCode, resp.Header, body, testModelID)
...
func (s *AccountTestService) testOpenAIAccountConnection(c *gin.Context, account *Account, modelID string, prompt string, mode string) error {
	...
	if resp.StatusCode != http.StatusOK {
		...
		s.applyTestUpstreamFailure(ctx, account, resp.StatusCode, resp.Header, body, modelID)
		return s.sendErrorAndEnd(c, fmt.Sprintf("API returned %d: %s", resp.StatusCode, string(body)))
	}
...
func (s *AccountTestService) testAntigravityAccountConnection(c *gin.Context, account *Account, modelID string) error {
	...
	if err != nil {
		if result != nil && result.StatusCode >= 400 {
			s.applyTestUpstreamFailure(ctx, account, result.StatusCode, result.Headers, result.Body, testModelID)
		}
		return s.sendErrorAndEnd(c, err.Error())
	}
