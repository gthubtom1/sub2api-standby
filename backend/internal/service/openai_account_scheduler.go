...
func isOpenAIAccountCandidateBetter(left openAIAccountCandidateScore, right openAIAccountCandidateScore) bool {
	...
	// Prefer recently standby-probed accounts so unattended failover picks healthy standbys first.
	if cmp := compareStandbyProbeFreshness(left.account, right.account, time.Now()); cmp != 0 {
		return cmp < 0
	}
