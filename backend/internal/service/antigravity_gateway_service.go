...
const (
	...
	antigravityRateLimitThreshold       = 3 * time.Second
...
type TestConnectionResult struct {
	...
	MappedModel string // mapped model id
	StatusCode  int
	Headers     http.Header
	Body        []byte
}
...
func (s *AntigravityGatewayService) TestConnection(ctx context.Context, account *Account, modelID string) (*TestConnectionResult, error) {
	...
	if result.resp.StatusCode >= 400 {
		return &TestConnectionResult{
			...
			Headers:     result.resp.Header.Clone(),
			Body:        respBody,
		}, fmt.Errorf("API returned %d: %s", result.resp.StatusCode, string(respBody))
	}
