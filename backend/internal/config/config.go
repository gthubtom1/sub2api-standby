...
type Config struct {
	...
	StandbyProbe            StandbyProbeConfig            `mapstructure:"standby_probe"`
...
// StandbyProbeConfig keeps pre-tested standby accounts ready for scheduling.
// When N accounts are actively used, the worker tries to keep N * StandbyPerActive
// other accounts connection-tested so failover does not burn client retries.
type StandbyProbeConfig struct {
	Enabled                   bool `mapstructure:"enabled"`
	CheckIntervalMinutes      int  `mapstructure:"check_interval_minutes"`
	ActiveWindowMinutes       int  `mapstructure:"active_window_minutes"`
	StandbyPerActive          int  `mapstructure:"standby_per_active"`
	HealthTTLMinutes          int  `mapstructure:"health_ttl_minutes"`
	Concurrency               int  `mapstructure:"concurrency"`
	MaxProbesPerCycle         int  `mapstructure:"max_probes_per_cycle"`
	CycleTimeoutSeconds       int  `mapstructure:"cycle_timeout_seconds"`
	DeficitIntervalSeconds    int  `mapstructure:"deficit_interval_seconds"`
	FirstDelaySeconds         int  `mapstructure:"first_delay_seconds"`
	RefreshBeforeTTLMinutes   int  `mapstructure:"refresh_before_ttl_minutes"`
	FailCooldownMinutes        int  `mapstructure:"fail_cooldown_minutes"`
}
...
func setDefaults() {
	...
	// StandbyProbe: proactive connection tests for standby scheduling accounts
	viper.SetDefault("standby_probe.enabled", true)
	viper.SetDefault("standby_probe.check_interval_minutes", 2)
	viper.SetDefault("standby_probe.active_window_minutes", 30)
	viper.SetDefault("standby_probe.standby_per_active", 2)
	viper.SetDefault("standby_probe.health_ttl_minutes", 15)
	viper.SetDefault("standby_probe.concurrency", 3)
	viper.SetDefault("standby_probe.max_probes_per_cycle", 40)
	viper.SetDefault("standby_probe.cycle_timeout_seconds", 240)
	viper.SetDefault("standby_probe.deficit_interval_seconds", 30)
	viper.SetDefault("standby_probe.first_delay_seconds", 5)
	viper.SetDefault("standby_probe.refresh_before_ttl_minutes", 5)
	viper.SetDefault("standby_probe.fail_cooldown_minutes", 5)
