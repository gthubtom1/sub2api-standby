...
/**
 * 鎵归噺鑾峰彇澶氫釜璐﹀彿鐨勪粖鏃ョ粺璁? * @param accountIds - 璐﹀彿 ID 鍒楄〃
 * @returns 浠ヨ处鍙?ID锛堝瓧绗︿覆锛変负閿殑缁熻鏄犲皠
 */
...
/**
 * Bulk test account connectivity.
 * Server tests each account concurrently and returns per-account results.
 * Max 100 IDs per request; callers should chunk larger selections.
 */
export interface BulkTestAccountResult {
  account_id: number
  name?: string
  platform?: string
...
export interface BulkTestAccountsResponse {
  total: number
  success: number
  failed: number
  results: BulkTestAccountResult[]
}

export async function bulkTest(
  accountIds: number[],
  options?: { model_id?: string; concurrency?: number; timeoutMs?: number }
): Promise<BulkTestAccountsResponse> {
  const { data } = await apiClient.post<BulkTestAccountsResponse>(
    '/admin/accounts/bulk-test',
    {
      account_ids: accountIds,
