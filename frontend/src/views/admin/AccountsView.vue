<template>
  <AppLayout>
        ...
        >
          ...
          <template #cell-status="{ row }">
            <div class="flex items-center gap-1.5">
              ...
              <AccountStandbyProbeBadge :account="row" />
            </div>
          </template>
    ...
    <BulkAccountTestModal
      :show="showBulkTest"
      :account-ids="bulkTestIds"
      :account-name-map="bulkTestNameMap"
...
import BulkAccountTestModal from '@/components/admin/account/BulkAccountTestModal.vue'
import AccountStatsModal from '@/components/admin/account/AccountStatsModal.vue'
import ScheduledTestsPanel from '@/components/admin/account/ScheduledTestsPanel.vue'
import type { SelectOption } from '@/components/common/Select.vue'
import AccountStatusIndicator from '@/components/account/AccountStatusIndicator.vue'
import AccountStandbyProbeBadge from '@/components/account/AccountStandbyProbeBadge.vue'
...
const bulkTestIds = ref<number[]>([])
const bulkTestNameMap = ref<Record<number, string>>({})
...
const openBulkTestModal = () => {
  if (selIds.value.length === 0) {
    appStore.showError(t('admin.accounts.bulkTest.noSelection'))
    return
  }
  const nameMap: Record<number, string> = {}
  for (const account of accounts.value) {
    if (selIds.value.includes(account.id)) {
      nameMap[account.id] = account.name
    }
  }
  bulkTestIds.value = [...selIds.value]
  bulkTestNameMap.value = nameMap
  showBulkTest.value = true
}
const closeBulkTestModal = () => {
  showBulkTest.value = false
  bulkTestIds.value = []
  bulkTestNameMap.value = {}
}
const handleBulkTestCompleted = (payload: { success: number; failed: number }) => {
  if (payload.failed > 0) {
    appStore.showError(t('admin.accounts.bulkActions.partialSuccess', { success: payload.success, failed: payload.failed }))
  } else {
    appStore.showSuccess(t('admin.accounts.bulkTest.completed', { count: payload.success }))
  }
  reload()
}
