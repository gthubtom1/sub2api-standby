...
> fork：`gthubtom1/sub2api-standby` · 镜像：`ghcr.io/gthubtom1/sub2api-standby:latest`  
> 完整教程：[docs/DEPLOY_STANDY_CN.md](docs/DEPLOY_STANDY_CN.md)

### 一键安装（推荐，只 pull，不编译）

与官方 Docker 一样：**自动安装，无安装向导**。脚本结束会打印管理员邮箱/密码，打开 `http://IP:8080` 登录。

```bash
curl -sSL https://raw.githubusercontent.com/gthubtom1/sub2api-standby/main/deploy/quick-pull-deploy.sh | bash
```

详细说明见 [docs/DEPLOY_STANDY_CN.md](docs/DEPLOY_STANDY_CN.md)。

或手动：

```bash
mkdir -p sub2api-standby && cd sub2api-standby
# 下载 compose（仓库 deploy/）
